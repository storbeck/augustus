// modules/augustus/pkg/generators/fireworks/config.go
package fireworks
...
// Config holds typed configuration for the Fireworks generator.
type Config struct {
	// Required
	Model  string
	APIKey string
...
func DefaultConfig() Config {
	return Config{
		Temperature: 0.7, // Match default from fireworks.go
	}
}
...
func ConfigFromMap(m registry.Config) (Config, error) {
	...
	if err != nil {
		return cfg, fmt.Errorf("fireworks generator requires 'model' configuration")
	}
	cfg.Model = model
	...
	if cfg.APIKey == "" {
		return cfg, fmt.Errorf("fireworks generator requires 'api_key' configuration or FIREWORKS_API_KEY environment variable")
	}
